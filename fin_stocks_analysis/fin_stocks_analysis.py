...
try:
    ...
    try:
        ...
        if os.path.isdir( curr_dir + r"\datebase" ) :
            
            if os.path.isdir( curr_dir + r"\datebase\%s" %(ticker1name) ):
                
                pass
            
            else:
                
                os.mkdir( curr_dir + r"\datebase\%s" %(ticker1name) )
                
            pass

        else:

            os.mkdir( curr_dir + r"\datebase" )
            os.mkdir( curr_dir + r"\datebase\%s" %(ticker1name) )
    ...
    data1StochOsc = getIndicators.getStochOsc(data1)
...
try:
    ...
    ax1_101.legend()
    ax1_101.grid(linestyle = "--", linewidth = "0.5")
    ...
    ax201.set_title("%s - DMAs through days %s - %s" %(ticker1name, startdate.date(), enddate.date()))
    ...
    fig12.canvas.manager.set_window_title("Stochastic Oscillator Chart")

    gs12 = fig12.add_gridspec(4,4)

    ax1201 = fig12.add_subplot(gs12[0:3,0:3])
    ax1201.set_title("%s - Stochastic Oscillator through days %s - %s" %(ticker1name, startdate.date(), enddate.date()))
    ohlcplot(data1, ax1201, fig12)
    plotkarde1201 = Drawer(ax1201,fig12)
    
    
    ax1202 = fig12.add_subplot(gs12[3:4, 0:3], sharex = ax1201)
    ax1202.plot(range(0, data1.index.size), data1StochOsc["StochOsc"], label = "Fast", color = "indianred")
    ax1202.plot(range(0, data1.index.size), data1StochOsc["StochOsc"].rolling(3).mean(), label = "Slow (3DMA)", color = "royalblue", alpha = 0.6)
    ax1202.axhline( data1StochOsc["StochOsc"].mean(), color= "blue")
    ax1202.axhline( 20, color = "green")
    ax1202.axhline( 80, color = "green")
    plotkarde1202 = Drawer(ax1202,fig12)
    ax1202.legend()
    
    
    mcursor12 = mplw.MultiCursor(fig12.canvas, [ax1201, ax1202] , horizOn= True, color = "lightskyblue" , linewidth = 1)
    fig12.tight_layout()

    #### -- E.O. Stochastic Oscillator -- ####
