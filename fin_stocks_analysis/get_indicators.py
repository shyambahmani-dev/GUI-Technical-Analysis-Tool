...
def getDMA(data1, intervals = [2, 3, 5, 7, 10, 15, 20, 25, 50, 65, 75, 100, 150, 200, 500]):
...
def getStochOsc(data1, interval = 14):
    
    data1StochOsc = pd.DataFrame(index = data1.index, columns = ["StochOsc"])
    
    for ind in range(data1.index.size):
        
        if( ind >= interval ):
            
            data1StochOsc["StochOsc"].iloc[ind] = ( (data1["Close"].iloc[ind] - data1["Low"].iloc[ind-interval:ind].min() )/ (data1["High"].iloc[ind-interval:ind].max() - data1["Low"].iloc[ind-interval:ind].min()) )
            data1StochOsc["StochOsc"].iloc[ind] *= 100
            
    return data1StochOsc

## -- Stochastic Oscillator Done -- ##